...
LoadObjFile( char *name )
{
	...
	for( ; ; )
	{
		...
		if( strcmp( cmd, "v" )  ==  0 )
		{
			...
			// Test if there's any more data (vertex colors)
			if ((str = strtok(NULL, OBJDELIMS)) != NULL)
			{
				sv.r = (float)atof(str);
			...
			else // if not, the default color is white
			{
				sv.r = 1.f;
				sv.g = 1.f;
				sv.b = 1.f;
		...
		if( strcmp( cmd, "f" )  ==  0 )
		{
			...
			for( int it = 0; it < numTriangles; it++ )
			{
				...
				for( int vtx = 0; vtx < 3 ; vtx++ )
				{
					...
					glColor3f((GLfloat)vp->r, (GLfloat)vp->g, (GLfloat)vp->b);
